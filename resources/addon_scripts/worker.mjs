...
const builders = {
	...
	playerGameModeChange: d => ({ player: P(d.player), get fromGameMode(){ return api._gameModeName(d.from); }, get toGameMode(){ return api._gameModeName(d.to); } }),
...
const beforeBuilders = {
	...
	playerGameModeChange: d => ({ player: P(d.player), get fromGameMode(){ return api._gameModeName(d.from); }, get toGameMode(){ return api._gameModeName(d.to); }, cancel: false }),
...
function handleSync(msg){
	let reply;
	switch(msg.t){
		case "before": reply = { t: "bres", ...before(msg) }; break;
		case "hook": reply = { t: "bres", ...hook(msg) }; break;
		case "pcall": reply = { t: "bres", ...pcall(msg) }; break;
		case "command": reply = { t: "bres", ...command(msg) }; break;
		default: reply = { t: "bres", e: "unexpected message " + msg.t }; break;
	}
	if (msg.reqId !== undefined) reply.reqId = msg.reqId;
	return reply;
}
...
async function init(msg){
	...
	for(const pack of msg.packs){
		const id = String(pack.id ?? pack.name);
		host.packNames[id] = String(pack.name);
		host.apiMajor[id] = pack.api ?? 2;
	}
	for(const pack of msg.packs){
		const id = String(pack.id ?? pack.name);
		try{
			await host.run(id, async () => {
				await import(pathToFileURL(pack.entry).href);
			}, "loading " + pack.entry);
		}catch(e){
			host.error(id, "loading " + pack.entry, e);
		}
	}
	await drain();
